 ...
 *
 * <p>
 * <em>Important! On most operating systems, you must have root access or
 * administrative privileges to use raw sockets.</em>
 * </p>
 */
public class RawSocket
{
	...
	/**
	 * A protocol family constant for {@link #open} indicating IPv4. <em>This should
	 * be moved to another class.</em>
	 */
	public static final int PF_INET;

	/**
	 * A protocol family constant for {@link #open} indicating IPv6. <em>This should
	 * be moved to another class.</em>
	 */
	public static final int PF_INET6;

	/**
	 * Initializes any system resources used by the RockSaw library. Really, all it
	 * does is call WSAStartup on Win32. It may be called multiple times (only the
	 * first call has any effect), but each call must be matched with a
	...
	/**
	 * Deallocates any system resources used by the RockSaw library. Really, all it
	 * does is call WSACleanup on Win32.
	 */
	...
	/**
	 * Writes a system error message into a StringBuffer. The message is appended to
	 * the supplied StringBuffer argument. This is a thread safe call on systems
	 * that support per-thread instances of errno.
	 *
	 * @param buffer
	 ...
	 *
	 * <p>
	 * <em>This method really belongs in another class, probably in vserv-tcpip, but
	 * is currently included here as a convenience. It may be moved elsewhere in the
	 * 1.0 release API.</em>
	 * </p>
	 *
	 * @return The protocol number corresponding to the given protocol name. If the
	 *         protocol name cannot be found, returns a negative value.
	 */
	 ...
	 *
	 * <p>
	 * <em>This method really belongs in another class,, but is currently included
	 * here for expediency. It may be moved elsewhere in the 1.0 release API.</em>
	 * </p>
	 *
	 * @param destination
	 ...
	 * with a destination matching the address to be delivered to the socket. Also,
	 * the kernel will set the source address of outbound packets to the bound
	 * address (unless {@link #setIPHeaderInclude setIPHeaderInclude(true)} has been
	 * called).
	 *
	 * @param address
	 *            The address to bind.
	...
	/**
	 * Sets or unsets the IP_HDRINCL socket option. Setting this option causes IPv4
	 * packet writes to expect the entire IP packet, starting from the header. The
	 * default behavior is to only expect the data payload. This option is valid
	 * only for IPv4 sockets.
	 *
	 * @param on
	...
	/**
	 * <p>
	 * Sets whether or not socket send/receive timeouts should be emulated by using
	 * the POSIX {@code select} function. Not all platforms support socket
	 * send/receive timeouts and this method provides a means to reproduce the same
	 * effect.
	 * </p>
	 *
	 * @param useSelect
	...
	/**
	 * <p>
	 * Determines whether or not socket send/receive timeouts are emulated by using
	 * the POSIX {@code select} system function. Not all platforms support socket
	 * send/receive timeouts. The default value is false except for platforms where
	 * the {@code
	 * os.name} property starts with the string "SunOS".
	 * </p>
	 *
	 * @return True if send/receive timeouts are emulated with select, false if not.
	 */
	...
	/**
	 * Sets the send timeout (SO_SNDTIMEO). A timeout of zero indicates an infinite
	 * timeout. A negative timeout is undefined.
	 *
	 * @param timeout
			...
			throws IllegalArgumentException, IOException, InterruptedIOException
	{
		...
		if (address != null
				&& ((__family == PF_INET && address.length != 4) || (__family == PF_INET6 && address.length != 16)))
	...
	/**
	 * Writes packet data to the socket. The data should not include the IP header.
	 * IPv4 ({@link #PF_INET}) sockets may set the IP_HDRINCL option with
	 * {@link #setIPHeaderInclude}, in which case the packet data should include the
	 * IP header.
	 *
	 * @param address
			...
			throws IllegalArgumentException, IOException, InterruptedIOException
	{
		...
		if (result == 0)
			result = __sendto(__socket, data, offset, length, __family, address.getAddress(), scope_id);
